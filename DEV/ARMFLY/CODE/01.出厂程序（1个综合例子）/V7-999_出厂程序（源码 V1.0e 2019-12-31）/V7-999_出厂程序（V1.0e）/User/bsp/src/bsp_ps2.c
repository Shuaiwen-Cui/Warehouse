...
*	文件名称 : bsp_ps2.c
*	版    本 : V1.0
*	说    明 : PS/2键盘鼠标驱动模块,自动识别键盘和鼠标
*
*	修改记录 :
*		版本号  日期         作者     说明
*		V1.0    2014-02-12   armfly  正式发布
*		V1.1    2019-02-23   armfly  移植到STM32H743，
*								- PS2_TZ增加DevMouse成员变量，用来做鼠标数据包解析
*								- 部分鼠标数据包间距6ms，调整包间超时为3ms
*
...
void PS2_StartWork(void)
{
	/* 配置 PS2_CLK 作为中断输入口，下降沿触发 */
	{
		...
		/* 2019-02-23 配置外EXIT过后，GPIO为输入，我们希望是开漏输出 */
...
void PS2_ISR(void)
{
	...
	if (g_tPS2.Sending == 0)	/* 主机接收键盘和鼠标数据的状态 */
	{
		...
		if (bsp_CheckRunTime(s_last_time) > 2)
		{
			g_tPS2.Status = 0;
		}
